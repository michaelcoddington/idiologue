...
public class ArrayBuffer {
    ...
    public ArrayBuffer.MatchResult match(int bufferStartPosition, byte[] testSequence) {
            ...
            if (array[i] == start) {
                ...
                for (; followPosition < testSequence.length && !mismatch; followPosition++) {
                    ...
                    if (bufferPosition < writePosition) {
                        ...
                        return new ArrayBuffer.PartialMatchResult(i - readPosition);
                    }
                }
                if (matchCount == testSequence.length) {
                    return new ArrayBuffer.CompleteMatchResult(i - readPosition, i - readPosition + matchCount - 1);
                }
            }
        }
        return new ArrayBuffer.NoMatchResult();
    }

    class MatchResult { }
    class NoMatchResult extends ArrayBuffer.MatchResult { }
    class CompleteMatchResult extends ArrayBuffer.MatchResult {
        int startPosition;
        int endPosition;
        ...
        public String toString() {
            return new StringJoiner(", ", ArrayBuffer.CompleteMatchResult.class.getSimpleName() + "[", "]")
                    .add("startPosition=" + startPosition)
                    .add("endPosition=" + endPosition)
                    .toString();
        }
    }
    class PartialMatchResult extends ArrayBuffer.MatchResult {
        int startPosition;
        ...
        @Override
        public String toString() {
            return new StringJoiner(", ", ArrayBuffer.PartialMatchResult.class.getSimpleName() + "[", "]")
                    .add("startPosition=" + startPosition)
                    .toString();
        }
    }
